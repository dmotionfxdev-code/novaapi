...
- **Bug fix: `Not valid JSON: 'utf-8' codec can't decode byte ...: invalid continuation byte` crash.** Root cause: nothing tied a scheduled `AcquisitionJob`'s `format` field to its `provider` — a GOOGLE_EARTH_ENGINE job could be scheduled with `format=JSON`/`GEOJSON`/`CSV`/`SHAPEFILE` (the general-purpose `ScheduleAcquisitionWizard` even defaults its format dropdown to `GEOJSON`, independent of the chosen provider) and would pass `AcquisitionJob.schedule()` cleanly. At execute time, `GoogleEarthEngineProvider`'s real raster bytes (a genuine GeoTIFF — always binary, never UTF-8 text) reached `domain/validation.py`'s `validate_json()`/`validate_geojson()`, whose `json.loads(content)` decodes `bytes` as UTF-8 internally and raised `UnicodeDecodeError` on the binary content, wrapped as `"Not valid JSON: {exc}"`. Traced end-to-end (frontend `ScheduleAcquisitionWizard.php`, backend `ScheduleAcquisitionJobRequest` schema, `AcquisitionJob.schedule()`) — confirmed this is a provider/format mismatch, not a GEE connectivity or GEE code defect (GEE's raster download itself was already proven working, including under real credentials, during the two GEE bug fixes above). Fixed with a schedule-time guard in `AcquisitionJob.schedule()`: GOOGLE_EARTH_ENGINE jobs must declare `format=GEOTIFF`, rejected immediately with a clear `InvalidAcquisitionJobError` otherwise — the same class of guard this method already applies for `remote_sensing_source`/`aoi_id`. Also added diagnostic logging in `gee_connector.py` (Content-Type header + hex/printable byte prefix) for the rare case a download's content doesn't start with a real TIFF header, so any future format/content-shape anomaly is diagnosable from logs rather than a bare stack trace. Verified with new unit tests: `test_data_acquisition_domain.py::test_schedule_gee_job_rejects_non_geotiff_format` (all four non-GEOTIFF formats rejected) and `test_gee_connector.py::test_fetch_logs_diagnostics_when_download_content_is_not_a_real_tiff`.
- **Bug fix: `Image.bitwiseAnd: Bitwise operands must be integer only` crash on CLOUD_MASKING.** Root cause: `_apply_preprocessing()` was applying cloud masking's QA-band `bitwiseAnd()` calls to the already-`.median()`-composited image, but Earth Engine's `.median()` reducer always returns FLOAT-typed bands for every band — including QA60 (Sentinel-2)/QA_PIXEL (Landsat) — regardless of the source bands' integer type, so the bitwise op on the (now-float) QA band crashed. This was purely a backend operation-ordering bug, not a frontend/band-selection/satellite-mismatch issue (traced end-to-end from `RemoteSensingPage.php` through `DataAcquisitionRepository.php`, both confirmed to send correctly-named preprocessing steps and bands). Fixed by moving cloud masking to run per-image via `ee.ImageCollection.map(...)` **before** `.median()` compositing (`_build_composite()` in `gee_connector.py`), which is also the semantically correct place for it — masking a composite after the fact never made sense (the point of cloud masking is to exclude cloud-contaminated pixels from the median in the first place). Verified with new unit tests proving the correct `.map()`-before-`.median()` call order for Sentinel-2 and Landsat, that masking is skipped (not faked) when not requested, and that MODIS honestly does not claim CLOUD_MASKING as applied (no usable QA band wired into this pipeline for it) — plus an extended real-network integration test requesting CLOUD_MASKING against the live Earth Engine API.
